...
| `kb_upgrade()` | Lemon Squeezy checkout link (after trial) |

Invite emails are optional (`INVITE_EMAIL_ENABLED` + Resend).

Billing: 14-day trial from first login, then `$19/mo` via Lemon Squeezy
(`LEMON_SQUEEZY_*` env + webhook `/billing/webhook`).
